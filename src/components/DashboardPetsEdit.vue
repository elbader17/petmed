  ...
  <div class="dashboard">
    <section v-if="showCard" class="credit-card" :class="renderPlans(pet.plan)">
      <div class="card-info">
        ...
        <div class="card-field">
          <span class="field-titles">Socio: <span class="extra-space" /> {{ pet.numAffiliate }}
            <span class="spaces" /> Plan: {{ pet.plan }}</span>
        </div>

        <div class="card-field">
          <span class="field-titles">fecha de nac:&nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp; {{ pet.birthdate }} &nbsp;&nbsp;
...
.pet-photo-container {
  position: relative;
  /* Agrega posicionamiento relativo al contenedor de la foto y el logo */
}
...
h1 {
  ...
  /* Added padding to create a background */
...
.code-container {
  ...
  flex-direction: column;
  align-items: center;
  /* Center the code text horizontally */
  margin-top: 1rem;
...
.logo-container2 {
  position: absolute;
  /* Agrega posicionamiento absoluto al logo */
  left: 10px;
  bottom: -220px;
...
.plan2 h1 {
  ...
  /* Cambia el color de fondo a un tono más oscuro */
}
...
.logo-container {
  position: absolute;
  /* Set the logo container to absolute positioning */
  left: 70%;
...
.logo2 {
  position: absolute;
  z-index: 0;
  /* Coloca la imagen detrás de otros elementos */
  width: 450px;
  right: 80%;
  bottom: 80px;
